#!/bin/bash
#SBATCH --job-name=QuickTestParallel
#SBATCH --output=%x.o%j
#SBATCH --error=%x.e%j
...
declare -a outArr=("parallel_short"
				   "parallel_short"
				   "parallel_short"
				   "parallel_short"
				   "parallel_short"
...
echo -e "Test Run Results:" > parallel_correctness.log
echo -e "Run Time Results:" > parallel_timing.log
...
for file in "$directory"/*; do
    if [[ $file =~ $pattern ]]; then
		...
		do
			...
			startTime=`date +%s.%N`
			python3 $file -i ${inArr[$i]} -o $outFile -s ${seedArr[$i]} -p ${procArr[$i]}  &> parallel_output.out
			endTime=`date +%s.%N`
			...
			if [ ${procArr[$i]} = "1" ]; then
				printf "%-40s" "    ${outArr[$i]} - ${procArr[$i]} process:" >> parallel_correctness.log
				printf "%-40s" "    ${outArr[$i]} - ${procArr[$i]} process:" >> parallel_timing.log
			else
				printf "%-40s" "    ${outArr[$i]} - ${procArr[$i]} processes:" >> parallel_correctness.log
				printf "%-40s" "    ${outArr[$i]} - ${procArr[$i]} processes:" >> parallel_timing.log
			fi
			printf "$correctness\n" >> parallel_correctness.log
